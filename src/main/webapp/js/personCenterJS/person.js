		...
		$.ajax({
		      ...
		      success:function(data){
		      	if(data.result!=null){
		      		...
		      		perInfo = {"userName":data.result.userName,
...
function changePerInfo(){
	...
	if(newUserName==perInfo.userName 
		...
		var changePerInfo={'userName':newUserName,
					       'userQq':newUserQq,
					       'userWx':newUserWx,
					       'userSex':newUserSex};
		if(returnPicAvatarName!=null){
...
function makeGoodsDetails(currentPage){
	if(currentPage != null){
		...
		if($("#souYiFaBuInput").val()!=null && $("#souYiFaBuInput").val()!=''){
			var data={"yiFaBuKind":nowYiFaBuClick,"currentPage":currentPage,"keyword":$("#souYiFaBuInput").val()};
		}else{
